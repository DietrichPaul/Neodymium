...
class Rotations(private val rotations: Array<Float> = arrayOf(0.0f, 0.0f)) {
    ...
    fun applySensitivity(client: MinecraftClient, previousRotations: Array<Float> = arrayOf(0.0f, 0.0f)): Rotations {
        ...
        return Rotations(ClientMathHelper.changeLookDirection(
            previousRotations,
            (ClientMathHelper.getAngleDifference(MathHelper.wrapDegrees(rotations[0]), MathHelper.wrapDegrees(rotations[1])) / 0.15) / fixedSensitivity,
            (ClientMathHelper.getAngleDifference(MathHelper.wrapDegrees(rotations[0]), MathHelper.wrapDegrees(rotations[1])) / 0.15) / fixedSensitivity,
        ))
    }
